...
int array[3] = {0};
int maxIndex = 0;
int default_signal_threshold = -113 ;
int current_signal_strength = 0;
int weak_start_change_operator_flag=0;
...
{
    int fd;
    const char *file_name = "/data/tywl/fm_weak_signal_threshold.cfg";
    char read_data[8] = {0};
    uint32_t len = 0;
    ...
    {
        default_signal_threshold = -113;//默认弱信号阈值
    }
...
int at_command_set_weak_operator_next(int current_index)
{
	static int ope_index = 0;
    FM_STATUS_t state = FM_SUCCESS;
    ope_index=current_index;
...
{   
   char current_operator[32] = {0};
    ...
    int signal_oprator_change_times =0;
	int cfd =0;
    int flag = 1;
...
{   sleep(2);
    ...
    weak_sleep = (weak_end_time - weak_start_time);
    if(weak_sleep>120)
    { printf("weak signal is weak up\n");
        flag=1;
    }
    while(flag)
   {
   ...
   printf("[%s], default signal_strength is: %d\n", __func__,default_signal_threshold);
   if(default_signal_threshold>-113 &&sim_switch_flag)
	{ 

		weak_start_change_operator_flag=1;
		fm_status=fm_get_signal_strength(&current_signal_strength);
		if(fm_status==FM_SUCCESS){
		sleep(5);
		
		printf("[%s], current signal_strength is: %d\n", __func__,current_signal_strength);
		
        int signal_qujian=current_signal_strength-default_signal_threshold;
		printf("[%s], default signal_qujian is: %d\n", __func__,signal_qujian);
       if (signal_qujian < 0 )
	{  
	   printf("[%s], current_signal_strength  is smaller\n", __func__);
	   signal_weak_times += 1;
	   printf("[%s], LOST TIME  is: %d\n", __func__,signal_weak_times);
	   if(signal_weak_times==1)
	   {

       fm_get_operator_index(current_operator,&current_index1);
	   array[current_index1]=current_signal_strength-default_signal_threshold;
       signal_weak_times=0;
	   if(signal_oprator_change_times<2){
       at_command_set_weak_operator_next(current_index1);
	   }
	   //reconnect();
	   signal_oprator_change_times++;

	   if(signal_oprator_change_times>2)
	   {
		signal_oprator_change_times=0;
		printf("[%s],signal_oprator_change_times is %d\n",__func__,signal_oprator_change_times);
		//default_signal_threshold=-140;
		
		maxIndex=findMaxIndex(array, 0, 1, 2);
		printf("[%s],in sleep 300,maxIndex is %d\n",__func__,maxIndex);
		if(maxIndex!=current_index1){
		fm_set_operator(oper_list[maxIndex]);
		printf("set max operator success\n");
        weak_start_change_operator_flag=0;
		weaksignalmainflag=0;
        flag = 0 ;
		sleep(10);
		
		
	    memset(array, 0, sizeof(array));
		printf("signal check finished\n");
		...
		weak_start_change_operator_flag=0;
		printf("[%s], current_signal_strength  is bigger\n", __func__);
    ...
    printf("weak signal change is closed\n");
   }
   sleep(20);
 }
 printf("weak signal is sleep\n");
              ...
              else
           {
               ...
               if (weakSignalValue == NULL) 
                ...
                {  
                    ...
                    if ( fm_status == FM_SUCCESS )
                    {
                    printf("[%s], save weak signal threshold SUCCESS , fm_status:[%d]\n", __func__, fm_status);
                    }else{
                    printf("[%s], save weak signal threshold ERROR , fm_status:[%d]\n", __func__, fm_status);
                    }
...
{   
  ...
  */
	...
	printf("ReceiveLoop TaskCreate done!");
		//MQTT重连检测机制
    printf("prepare start  weak task done!\n");

    ret=fm_get_weak_signal_threshold();
	if (ret==FM_SUCCESS)
	{
       printf("get_default _weak_signal_success\n");
	}else {
       printf("get_weak_signal_faiil\n"); 
	}
