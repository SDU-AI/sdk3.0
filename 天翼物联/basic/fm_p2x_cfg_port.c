...
FM_STATUS_t fm_get_lost_network_policy_status(FM_POLICY_STATUS_t *policy_status) 
{
    ...
    uint32_t len = 0;
    char read_data[8] = {0};
    const char *file_name = "/etc/config/fm_lost_network_policy_status.cfg";
...
FM_STATUS_t fm_set_weak_signal_threshold(int signal_threshold)
{
    int fd;
    int ret = 0;
    const char *file_name = "/etc/config/weak_signal_threshold.cfg";
    uint32_t wirte_len = 0;
...

{
	...
	int ret = 0;
	int32_t fd;
    const char *file_name = "/etc/config/fm_prefer_operator_policy.cfg";
    int flags = FM_FS_WB;
