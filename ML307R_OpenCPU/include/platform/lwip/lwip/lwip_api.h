...
typedef struct {
    ...
    u8_t lwip_iot_test_mode; /*0: normal mode; 1, iot test mode; 2, minidp test mode*/
...
int lwip_mq_left_query(int type);
...
/*set proxy, addr and port, and will enable proxy function auto*/
void lwip_set_proxy_func(ip_addr_t * addr, u16_t port);
/*set proxy addr parameter only*/
void lwip_set_proxy_addr(ip_addr_t * addr);
/*set proxy port parameter only*/
void lwip_set_proxy_port(u16_t port);
/*set proxy flag parameter, 0 will disable procy, > 0 enable proxy*/
...
void lwip_set_icmp_dont_frag_flag(int flag);
int lwip_get_icmp_dont_frag_flag(void);
...
void lwip_set_ethip6_ns_pool_first(int flag);
int lwip_get_ethip6_ns_pool_first(void);
void lwip_set_ip6_enable_flag(int flag);
int lwip_get_ip6_enable_flag(void);
...
/*Note: tcp nat ttl set, default set to LWIP_NAT_TTL_SECONDS_TCP = 600 ms */
/* The value set is ms. if set to -1, denote persistant allways for tcp nat entry*/
void lwip_set_nat_ttl_tcp_set(int value);
int lwip_get_nat_ttl_tcp_set(void);

void lwip_set_ipnet_mcpy_shift(int flag);
int lwip_get_ipnet_mcpy_shift(void);
...
void lwip_set_tcp_ack_now_(int value);
void lwip_set_tcp_man_wndsc(u8_t value);
void lwip_set_tcp_man_window(u16_t value);
void lwip_set_tcp_man_window_ul(u16_t value);
void lwip_set_wndsc_man_flag(int flag);
int lwip_get_wndsc_man_flag(void);
...
/*Note: tcp syn rto factor, control the first syn rto timer*/
/*default set to 3, just denote rto timer * 3.*/
void lwip_set_tcp_rto_syn_factor(int value);
int lwip_get_tcp_rto_syn_factor(void);
...
void lwip_set_ip_default_ttl_in(u8_t value);
u8_t lwip_get_ip_default_ttl_in(void);
...
int lwip_check_forward6_for_pbuf(struct pbuf* p);
...
void lwip_iot_test_mode_env(u8_t testmode);
